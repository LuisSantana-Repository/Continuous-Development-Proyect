  ...
  const fetchProvider = useCallback(async () => {
    try {
      ...
      const providersResponse = await fetch(
        `/providers/user/${profile.user.user_id}`,
        {
          method: 'GET',
      ...
      try {
        const ratingResponse = await fetch(
          `/reviews/provider/${providerInfo.provider_id}/rating`,
          {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
          }
      ...
      try {
        const requestsResponse = await fetch(
          `/service-requests/provider/${providerInfo.provider_id}?status=completed`,
          {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
