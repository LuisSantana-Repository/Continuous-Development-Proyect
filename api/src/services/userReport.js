 ...
 * @param {string} data.userId - User ID creating the report
...
export async function createUserReport(data) {
  ...
  // Verify service request exists and belongs to the user
  ...
  await db.execute(
    `INSERT INTO user_reports 
      (report_id, user_id, service_request_id, report_message, status) 
     VALUES (?, ?, ?, ?, 'pending')`,
    [reportId, userId, serviceRequestId, reportMessage]
  );
...
/**
 * Get reports by user ID (reports created by a user) with pagination
...
export async function getUserReports(
  ...
  console.log("getUserReports params:", {
...
/**
 * Get reports about a provider (reports targeting a provider) with pagination
 ...
 */
export async function updateUserReportStatus(reportId, updates) {
  const db = await getPrimaryPool();
...
export async function deleteUserReport(reportId) {
  ...
  const [result] = await db.execute(
    "DELETE FROM user_reports WHERE report_id = ?",
    [reportId]
  );
 ...
 */
export async function getProviderReportStats(providerId) {
  const db = await getPrimaryPool();
